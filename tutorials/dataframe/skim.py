...
def skim(mode):
    ...
    for s in samples:
        ...
        if mode == 'nominal':
            ...
            pt_cut = 0.4
        elif mode == 'ptcut_down':
            pt_cut = 0.2
        df[s] = df[s].Define("good_lep", "abs(lep_eta) < 2.5 && lep_pt > 5000 && lep_ptcone30 / lep_pt < {PTCUT} && lep_etcone20 / lep_pt < {PTCUT}".format(PTCUT=pt_cut))\
    ...
    for s in samples:
            tag = '' if mode == 'nominal' else '.' + mode
            df[s].Snapshot('mini', s + '.4lep{}.root'.format(tag),
                    ('goodlep_pt', 'goodlep_eta', 'goodlep_phi', 'goodlep_E', 'met_et', 'weight'))
