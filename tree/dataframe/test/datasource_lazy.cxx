...
TEST(RLazyDS, Constructor)
{
   ...
   RLazyDS<RResultPtr, double, float> tds({col0Name, col0}, {col1Name, col1});
...
TEST(RLazyDS, RangesOneSlot)
{
   ...
   auto col0Init = 0.;
   auto col0 = d.Define(col0Name, [&col0Init]() { return col0Init += 1.; }).Take<double>(col0Name);
   RLazyDS<RResultPtr, double> tds({col0Name, col0});
...
TEST(RLazyDS, ColSizesCheck)
{
   ...
   RLazyDS<RResultPtr, double, float> tds({"zero", col0}, {"one", col1});
   tds.SetNSlots(4);
   int ret(1);
   try {
      tds.Initialise();
